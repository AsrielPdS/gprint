...
abstract class Box<L = unknown, T extends IBox<L> = Boxes<L>, P extends BoxParent<T> = BoxParent<any>> implements Scope {
  ...
  addBox(s: S, index: number) {
    this.parts[this.end = index] = s.c([C.box]);
  ...
  write(pag: number) {
    ...
    if (l) {
      if (map && this.mode != BoxMode.editing) {
        ...
        if (!ctx || typeof ctx != "object" || !('length' in ctx)) {
          // @if DEBUG
...
class SymbolBox<L = unknown> extends Box<L, ISymbolBox<L>> {
  ...
  valid(index) {
    ...
    if (this.data !== undefined)
      ...
      data = this.data = super.valid(index) && this.ctx.rsc[src];
      if (data) {
        normBox(data);
        //data.$.id = model.$.id;
    ...
    append(dt, index) {
      if (index != currentPag) {
        ...
        if (bk.top) {
          ...
          h -= bk.top.$.part(index).e.offsetHeight;
        }
        content.add(bd);
        if (bk.bottom) {
          write(bk.bottom, index, DTParts.b, p);
          h -= bk.bottom.$.parts[index].e.offsetHeight;
        }
...
export function normalize(dt: Book) {
  normBox(dt.dt);
  dt.hd && normBox(dt.hd);
  dt.top && normBox(dt.top);
  dt.bottom && normBox(dt.bottom);
  dt.ft && normBox(dt.ft);
